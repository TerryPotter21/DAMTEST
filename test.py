...
if is_code_valid:
    ...
    status_placeholder.write("Loading DAM Monthly Data. Please wait...")
    ...
    # Display immediately whether the data is current
    data_status_placeholder.write(f"**Model using current monthly data: {is_current_data}**")
    ...
    if st.button("Proceed"):
        ...
        for ticker in tickers:
            ticker_status.text(f"Downloading data for {ticker}...")
            stock = yf.Ticker(ticker)
            data = stock.history(period='14mo', interval='1mo')
            data.reset_index(inplace=True)
